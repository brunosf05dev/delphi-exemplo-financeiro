...
procedure TFUser.BtnReportClick(Sender: TObject);
begin
  DM.frxReportGeral.Clear;
  if (FileExists(ExtractFilePath(ParamStr(0)) + 'Report\User.Listing.fr3')) then
    DM.frxReportGeral.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'Report\User.Listing.fr3')
  else
    Application.MessageBox('O arquivo do relatório não foi encontrado! Tente atualizar seu relatório','Arquivo não encontrado',0);


  DM.frxDBDatasetUser.DataSource:= DM.DSUser;
...
procedure TFUser.GDataDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  if DM.SQLUser.Active = False then
    Exit;
